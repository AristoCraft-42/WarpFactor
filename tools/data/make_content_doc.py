...
w("- Карта планеты складывается из областей разного пола, скальных гряд с проходами, озёр (если узлу")
w("  выпала вода) и рудных полей по нескольку залежей: у каждой руды своя сторона карты, а первая залежь")
w("  каждой — рядом с посадкой. К каждому полю генератор гарантирует проход по земле, враги приходят с краёв.")
w("- Типы планет (world/planet_types/*.tres): обычная, рудный мир (руды больше, волны раньше и злее)")
w("  и пустошь (ни руд, ни врагов). Размеры, доля скал и число озёр — в описании типа.")
