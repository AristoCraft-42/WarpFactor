...
## Спрайт здания — полоса квадратных кадров состояний слева направо:
##   0 — работает, 1 — простаивает (всё есть, но выход забит), 2 — выключено
##   (нет топлива, электричества или сырья).
## Сторона кадра — size × 32 пикселя. Кадров может быть меньше трёх: недостающие
## берутся из первого, поэтому один кадр работает как раньше.
