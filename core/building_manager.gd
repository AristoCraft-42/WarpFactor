...
enum Check {
	...
	LIFT_AREA, ## лифт — только на площадке шлюза или в открытой части подземного этажа (выставляет Run)
	LIFT_PAIR, ## на другом этаже на месте пары лифта занято (выставляет Run)
	LOCKED, ## закрыто исследованием (выставляет Run)
}
