...
В конце — счётчик (сейчас **1962 проверки**) и ненулевой код возврата при провале.
...
и делает скриншот на каждом шаге. Сейчас **229 проверок**, около 8 минут. Флаг `--autoshot` сам включает читы: сценарий нажимает
...
Кладёт в `D:/shots` по картинке на каждый тип планеты: пол, руды, озёра и точки появления врагов.
