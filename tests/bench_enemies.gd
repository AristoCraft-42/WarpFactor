extends Node
## Бенчмарк боя: планета 256×192 со скалами, ~5000 зданий (ленты и склады), 1000 врагов идут к шлюзу.
...
func _ready() -> void:
	...
	var spawned := 0
	var guard := 0
	while spawned < ENEMIES and guard < ENEMIES * 20:
		guard += 1
		var tile := Vector2i(rng.randi_range(2, 253), rng.randi_range(2, 189))
		var edge := mini(mini(tile.x, tile.y), mini(255 - tile.x, 191 - tile.y))
		if edge > 30 or planet.flow.get_dist(tile) >= FlowField.INF or planet.flow.blocked[tile.y * 256 + tile.x] != FlowField.OPEN:
			continue
	...
	print("  шлюз получил урон: %s, разрушено построек: %d" % [str(planet.gateway.is_damaged() or planet.gateway.health < 1.0e12), planet.destroyed_count])
	run.dispose()
	get_tree().quit()
